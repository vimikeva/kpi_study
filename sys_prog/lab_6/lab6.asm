...
max_prg		equ	10		;максимальна кількість "одночасно"
					; виконуваних  задач
time_slice	equ	4096		; кількість мікросекунд, виділених на один
...
; (варіант 3)
	pushf                     	;програмна імітація апаратного переривання
					;ВІДМІТИМО - ознака дозволу на переривання 
					;(if) апаратурою скинута в 0.

	call	cs:dword ptr int8ptr	;виклик системної процедури обробки переривання int8,
...
disp005:				; перервана задача фонова
	...
	mov	cx,max_prg-1            	; max_prg - max кількість задач
	jmp	disp015
...
disp015:				; визначення задачі, якій необхідно передати управління
	mov	di,max_prg+1
	sub	di,cx
	add    	di,nprg
	cmp    	di,max_prg
...
disp100:				; першопочатковий запуск задачі
	mov	nprg,bx
	mov	sp,stp[ebx*2]
	mov	al,clock[bx]
	mov	clockt[bx],al		; встановити дозволену
	...
	;xor   	esi,esi
	movzx  	esi,nprg		; на ax - номер задачі, яка
...
;  - розміщує в цю область по адресі [bp-2] залишок від ділення
