...
(defun initialization-tables ()
  ...
  (dolist (elem *keywords*)
    (setf (gethash (hash-table-count *keyword-and-alocd-table*) *keyword-and-alocd-table*) elem))
...
(defun character-token (stream syms pos token-list)
  (let ((sym-next (scan stream)))
    (if (member (aref *ascii-code-table* (char-code sym-next)) '(0 1))
